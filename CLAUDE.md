...
The block above is auto-maintained by `docops init` and `docops upgrade`.
Edit content outside the `<!-- docops:* -->` delimiters freely; DocOps will
preserve it. Edits inside the delimiters will be overwritten on the next
upgrade. The same docops block also appears in `AGENTS.md` (the multi-tool
sibling); both refresh from the same shipped template.
